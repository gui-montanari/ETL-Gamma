...
"""
Módulo de carregamento de dados para o KPI de Fechamento de Comissão (meses passados).

Este módulo contém funções para carregar os dados de fechamento de comissão
...
from utils.db_schema_farmer.db_schema_comissao import create_fechamento_comissao_farmer_table
...
def load_fechamento_comissao_farmer(df_fechamento, farmer_id=None):
    ...
    try:
        ...
        with DatabaseConnection() as conn:
            ...
            if not create_fechamento_comissao_farmer_table(conn):
            ...
            with conn.cursor() as cursor:
                ...
                if farmer_id:
                    ...
                    cursor.execute("""
                    DELETE FROM analysis.fechamento_comissao_farmer 
                    WHERE mes = %s 
                    AND farmer_id = %s
                    AND is_current_month = FALSE
                ...
                for _, row in df_fechamento.iterrows():
                    ...
                    data_to_insert.append((
                        ...
                        row['periodo_responsabilidade_inicio'] if pd.notna(row['periodo_responsabilidade_inicio']) else None,
                        row['periodo_responsabilidade_fim'] if pd.notna(row['periodo_responsabilidade_fim']) else None,
                        row['churn_total'],
                        row['meta_churn'],
                        row['status_churn'],
                ...
                if data_to_insert:
                    cursor.executemany("""
                    INSERT INTO analysis.fechamento_comissao_farmer
                    (mes, mes_formatado, farmer_id, farmer_name, hierarchy_level, 
                    data_positivador, periodo_responsabilidade_inicio, periodo_responsabilidade_fim,
